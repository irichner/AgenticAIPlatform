...
import { Bot, Play, MoreHorizontal, Server } from "lucide-react";
    ...
    >
      ...
      {mcpCount > 0 && (
        <div
          ...
          title={d.mcpNames?.join(", ")}
        >
          <Server className="w-3 h-3 text-violet/60 shrink-0" />
          <span className="text-[10px] text-text-3 truncate">
            {mcpCount} MCP{mcpCount !== 1 ? "s" : ""}
            {d.mcpNames && d.mcpNames.length > 0 && (
              <span className="text-text-3/60"> · {d.mcpNames.slice(0, 2).join(", ")}{d.mcpNames.length > 2 ? "…" : ""}</span>
            )}
          </span>
        </div>
      )}
