...
class Run(Base, TimestampMixin):
    ...
    agent_version_id: Mapped[uuid.UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("lanara.agent_versions.id", ondelete="SET NULL"), nullable=True
    )
    business_unit_id: Mapped[uuid.UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("lanara.business_units.id", ondelete="SET NULL"), nullable=True
    )
